...
async function fetchData() {
  const apiUrl = 'https://cms.fathforce.com';
  const defaultHero = {
  ...
  try {
    const [heroData, featureData, faqData, reasonsData, testimonialsData] = await Promise.all([
      getData(apiUrl).catch(() => defaultHero),
      getData(apiUrl).catch(() => defaultFeature),
      getData(apiUrl).catch(() => defaultFAQ),
      getData(apiUrl).catch(() => defaultReasons),
      getData(apiUrl).catch(() => defaultTestimonials)
    ]);
    return { heroData, featureData, faqData, reasonsData, testimonialsData };
  } catch (error) {
    console.error('Error fetching data:', error);
    return { heroData: defaultHero, featureData: defaultFeature, faqData: defaultFAQ, reasonsData: defaultReasons, testimonialsData: defaultTestimonials };
  }
}

export default async function Home() {
  const { heroData, featureData, faqData, reasonsData, testimonialsData } = await fetchData();
