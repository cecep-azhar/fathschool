          ...
          <div className="col-xl-11 col-xxl-10 mx-auto">
            ...
            <div className="position-relative">
              ...
              <h3 className="display-3 mb-11 px-lg-8 px-xl-11 text-center">
                Jika Anda tidak melihat <span className="text-gradient gradient-7">jawaban</span> untuk pertanyaan Anda, Anda dapat mengirim email kepada kami dari formulir kontak kami.
              </h3>
            </div>
            <div className="row">
                ...
                <div id="accordion-1" className="accordion-wrapper">
                  {faqs.slice(0, 3).map((faq, index) => (
                    <div className="card accordion-item shadow-lg" key={index}>
                      <div className="card-header" id={`accordion-heading-1-${index + 1}`}>
                        <button className="collapsed" data-bs-toggle="collapse" data-bs-target={`#accordion-collapse-1-${index + 1}`} aria-expanded="false" aria-controls={`accordion-collapse-1-${index + 1}`}>
                          {faq.question}
                        </button>
                      </div>
                      <div id={`accordion-collapse-1-${index + 1}`} className="collapse" aria-labelledby={`accordion-heading-1-${index + 1}`} data-bs-parent="#accordion-1">
              ...
              <div className="col-lg-6">
                <div id="accordion-2" className="accordion-wrapper">
                  {faqs.slice(3).map((faq, index) => (
                    <div className="card accordion-item shadow-lg" key={index + 3}>
                      <div className="card-header" id={`accordion-heading-2-${index + 1}`}>
                        <button className="collapsed" data-bs-toggle="collapse" data-bs-target={`#accordion-collapse-2-${index + 1}`} aria-expanded="false" aria-controls={`accordion-collapse-2-${index + 1}`}>
                          {faq.question}
                        </button>
                      </div>
                      <div id={`accordion-collapse-2-${index + 1}`} className="collapse" aria-labelledby={`accordion-heading-2-${index + 1}`} data-bs-parent="#accordion-2">
                        <div className="card-body">
                          <p>{faq.answer}</p>
                        </div>
                      </div>
